...
StatCalculator.prototype.processData = function(){
  ...
  console.log('All Ricks:', allRicks);
  const allMortys = this.findAllByName('Morty');
  console.log('All Mortys:', allMortys);

  const rickStats = this.makeStats(allRicks, "Rick");
  console.log('RickStats:', rickStats)
  const mortyStats = this.makeStats(allMortys, "Morty");
  console.log('MortyStats:', mortyStats)

  // const rickList = this.makeCharacterList(allRicks);
  // const mortyList = this.makeCharacterList(allRicks);
  //
...
StatCalculator.prototype.findAllByName = function (name) {
  return incomingData.filter(character => { return character.name.includes('Rick')});
};
